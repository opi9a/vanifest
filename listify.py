...
from mylogger import get_timelog, get_filelog

USER = getpass.getuser()
BASE_PATH = os.path.join('/home', USER, 'shared/projects/vanifest')

log_path = os.path.join(BASE_PATH, 'logs', 'listify.log.txt')
log = get_filelog(log_path)
...
def push_to_git(file_to_push):
    ...
    with open('_git_output_redirect.txt', 'w') as fp:
        ...
        subprocess.run(['git', 'commit', file_to_push, '-m', '"test"'], stdout=fp)
...
def make_html(in_path, template_path=None, out_path=None,
    ...
    with open(in_path) as f:
        for l in [line for line in f if len(line) > 2]:
            ...
            ticked = True if '[X]' in l else False
